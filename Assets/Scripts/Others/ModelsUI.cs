...
public class ModelsUI : LineDrawer
{
    ...
    public TapPanels[] panels = new TapPanels[(int)UIType.Parts];
    ...
    {
        //选择按钮添加事件监听
        panels[(int)UIType.Parts].button.OnDownEvent.AddListener(() => { SetPanelInteractive(UIType.Parts); });
        panels[(int)UIType.Tools].button.OnDownEvent.AddListener(() => { SetPanelInteractive(UIType.Tools); });

        //两个个界面的子物体添加交互
        for (int i = 0; i < (int)UIType.Tools; i++)
        {
        ...
        //默认打开家具
        SetPanelInteractive(UIType.Parts);
    }
