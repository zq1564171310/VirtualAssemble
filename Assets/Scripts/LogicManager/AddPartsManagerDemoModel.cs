...
namespace WyzLink.LogicManager
{
    ...
    public class AddPartsManagerDemoModel : Singleton<AddPartsManagerDemoModel>, AddPartsManager
    {
        ...
        public void Init()
        {
            ...
            if (null != RootPartGameObject)
            {
                ...
                {
                    var _NodesController = new GameObject("NodesController", typeof(NodesControllerDemoModel));
                    _NodesController.transform.parent = _RuntimeObjectNodes.transform;
                }
                if (null == NodesCommonDemoModel.Instance)
                {
                    var _NodesCommon = new GameObject("NodesCommon", typeof(NodesCommonDemoModel));
            ...
            if (null != PartsTransform)
            {
                ...
                foreach (Transform child in PartsTransform)
                {
                    if (null != child.GetComponent<Node>())
                    {
                        ...
                        else
                        {
                            ...
                            node.SetInstallationState((InstallationState)PlayerPrefs.GetInt(node.nodeId.ToString()+"DemoModel"));
                        }
                        ...
                        NodesControllerDemoModel.Instance.AddNodeList(node);
                    }
                }
                float ScalingNum = 1;
                for (int i = 0; i < NodesControllerDemoModel.Instance.GetNodeList().Count; i++)
                {
                    if (null != NodesControllerDemoModel.Instance.GetNodeList()[i].GetComponent<MeshFilter>())
                    {
                        ScalingNum = NodesControllerDemoModel.Instance.GetNodeList()[i].Scaling(ModelType.Part);
                        NodesControllerDemoModel.Instance.GetNodeList()[i].gameObject.transform.localScale /= ScalingNum;
                        if (NodesControllerDemoModel.Instance.GetNodeList()[i].gameObject.transform.name == "抽屉组件")
                        {
                            NodesControllerDemoModel.Instance.GetNodeList()[i].gameObject.transform.localScale /= 10;
                        }
                        if (NodesControllerDemoModel.Instance.GetNodeList()[i].gameObject.transform.name == "外壳组件")
                        {
                            NodesControllerDemoModel.Instance.GetNodeList()[i].gameObject.transform.localScale /= 10;
                        }
                    }
                ...
                if (null == AssembleManagerDemoModel.Instance)
                {
                    var _AssembleManager = new GameObject("AssembleManager", typeof(AssembleManagerDemoModel));
                    _AssembleManager.transform.parent = _RuntimeObjectNodes.transform;
                    _AssembleManager.GetComponent<AssembleManagerDemoModel>().SetDependencyGraph(new DependencyGraph(RootPartGameObject.GetComponent<AssembleController>(), RootPartGameObject.GetComponent<AssembleController>().assembleFlow.text));
                }
            }
        }
