...
using Windows.Storage;
    ...
    public class AssembleManagerExamModel : Singleton<AssembleManagerExamModel>, AssembleManager
    {
        ...
        private UIPartsPanelClassExamModel _UIPartsPanelClass;
        ...
        public void Init()
        {
            ...
            {
                if (NodesControllerExamModel.Instance.GetNodeList()[i].GetInstallationState() == InstallationState.Installed)
                {
                    InstalledNodeList.Add(NodesControllerExamModel.Instance.GetNodeList()[i]);
                }

                if (NodesControllerExamModel.Instance.GetNodeList()[i].GetInstallationState() == InstallationState.NextInstalling)
                {
                    NextInstallNodeList.Add(NodesControllerExamModel.Instance.GetNodeList()[i]);
                }
            }
        ...
        public void NextInstall(Node node)
        {
            ...
            if (NextInstallNodeList.Count == 0)
            {
                for (int i = 0; i < NodesCommonExamModel.Instance.GetNodesList().Count; i++)
                {
                    if (InstallationState.NotInstalled == NodesCommonExamModel.Instance.GetNodesList()[i].GetInstallationState())
                    {
                        NextInstallNodeList.Add(NodesCommonExamModel.Instance.GetNodesList()[i]);
                        NodesCommonExamModel.Instance.GetNodesList()[i].SetInstallationState(InstallationState.NextInstalling);
                        int index = _UIPartsPage.GetIndex(NodesCommonExamModel.Instance.GetNodesList()[i]);
                        NextParts.text = NodesCommonExamModel.Instance.GetNodesList()[i].name + "(第" + index + "页）" + "/";
                        break;
                    }
                }
            }
        ...
        public void LastNodeClick()
        {
            ...
            {
                NodesCommonExamModel.Instance.SetInstallationState(InstalledNodeList[InstalledNodeList.Count - 1].nodeId, InstallationState.NextInstalling);                //回退之前设置一下安装状态
                ...
                if (null != NodesControllerExamModel.Instance.GetNodeList())
                {
                    string tips = "";
                    int index = 1;
                    foreach (Node no in NodesControllerExamModel.Instance.GetNodeList())
                    {
            ...
            {
                NodesCommonExamModel.Instance.SetInstallationState(InstalledNodeList[InstalledNodeList.Count - 1].nodeId, InstallationState.Step1Installed);                //回退之前设置一下安装状态
        ...
        public void SkipClick()
        {
            ...
            {
                foreach (Node nodeContr in NodesControllerExamModel.Instance.GetNodeList())
                {
                    if (InstallationState.NextInstalling == nodeContr.GetInstallationState())
                    {
                        NodesCommonExamModel.Instance.SetInstallationState(nodeContr.nodeId, InstallationState.Step1Installed);                //跳过之前设置一下安装状态
                        ...
                        gameobj.AddComponent<NodeManagerExamModel>();
        ...
        {
            for (int i = 1; i < NodesCommonExamModel.Instance.GetNodesList().Count; i++)
            {
                NodesCommonExamModel.Instance.GetNodesList()[i].EndPos = scalNum * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale - ScaleCenter) + ScaleCenter;
            }
        ...
        public void WorkSpaceRota(int rota)
        {
            ...
            if (0 == rota)        //左旋转
            {
                for (int i = 1; i < NodesCommonExamModel.Instance.GetNodesList().Count; i++)   //因为第一个物体是底座，所以他不在零件架上，直接出现在安装区域，所以应该pas
                {
                    NodesCommonExamModel.Instance.GetNodesList()[i].EndPos = Quaternion.AngleAxis(-WorkSpaceRotaAngle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPos - RotaAngleCenter) + RotaAngleCenter;
                    NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale = Quaternion.AngleAxis(-WorkSpaceRotaAngle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale - RotaAngleCenter) + RotaAngleCenter;
                }
        ...
        public void AutoScaleAndRota(Node node)
        {
            ...
            for (int i = 1; i < NodesCommonExamModel.Instance.GetNodesList().Count; i++)   //因为第一个物体是底座，所以他不在零件架上，直接出现在安装区域，所以应该pas
            {
                NodesCommonExamModel.Instance.GetNodesList()[i].EndPos = Quaternion.AngleAxis(-angle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPos - RotaAngleCenter) + RotaAngleCenter;
                NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale = Quaternion.AngleAxis(-angle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale - RotaAngleCenter) + RotaAngleCenter;
            }
        ...
        public void Recovery()
        {
            for (int i = 1; i < NodesCommonExamModel.Instance.GetNodesList().Count; i++)   //因为第一个物体是底座，所以他不在零件架上，直接出现在安装区域，所以应该pas
            {
                NodesCommonExamModel.Instance.GetNodesList()[i].EndPos = Quaternion.AngleAxis(-RotaAngle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPos - RotaAngleCenter) + RotaAngleCenter;
                NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale = Quaternion.AngleAxis(-RotaAngle, Vector3.up) * (NodesCommonExamModel.Instance.GetNodesList()[i].EndPosForScale - RotaAngleCenter) + RotaAngleCenter;
            }
